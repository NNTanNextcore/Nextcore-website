...
try{
 ...
 for(const route of routes){
  ...
  await b.send('Page.navigate',{url:'http://localhost'+route});await b.send('Page.bringToFront');await delay(1500);
  ...
  const dom=await b.evaluate(`(()=>{
   ...
   return {duplicateIds:ids.filter((id,i)=>ids.indexOf(id)!==i),emptyIds:ids.filter(id=>!id).length,
    ...
    untranslatedHomepage:location.pathname==='/en/'?[...document.querySelectorAll('main *')].filter(e=>!e.children.length&&/[ăâđêôơưạảãậắếệịọớợụứựỳỹ]/i.test(e.textContent)).map(e=>({tag:e.tagName,cls:e.className,text:e.textContent.trim().slice(0,250)})):[]};})()`);
