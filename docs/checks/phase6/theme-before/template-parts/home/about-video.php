    ...
    data-light-src="<?php echo esc_url(nextcore_mode_url('nc_about_video_light', 'video/caurongquay-light-video.mp4', true)); ?>"></video>
      <div class="container company-video-inner">
        <div class="company-video-copy">
          ...
          <p class="company-founded" data-reveal><?php esc_html_e('Thành lập vào', 'nextcore-theme'); ?> <?php $founded = nextcore_field('nc_about_founded_date', '2022-06-15'); $date = DateTimeImmutable::createFromFormat('!Y-m-d', $founded); if (!$date || $date->format('Y-m-d') !== $founded) { $founded = '2022-06-15'; $date = new DateTimeImmutable($founded); } ?><time datetime="<?php echo esc_attr($founded); ?>"><?php echo esc_html(wp_date(__('j \\t\\h\\g n, Y', 'nextcore-theme'), $date->getTimestamp())); ?></time></p>
          <div class="company-description" data-reveal><?php echo wp_kses(nextcore_field('nc_about_body', '<p>Chuyên thực hiện phát triển, bảo trì các dự án CNTT cho các đối tác outsource.</p>
