...
class SnapWP
{
    ...
    public static function dbDelta($queries = '', $execute = true)
    {
        ...
        if (strlen($unexpectedOutput)) {
            error_log("DB DELTA UNEXPECTED OUTPUT\n----------" . $unexpectedOutput . "\n----------");
        }
    ...
    public static function getSitesIds()
    {
        ...
        return self::getSites(array('fields' => 'ids'));
    }
     ...
     * @return array<string, mixed[]>
     */
    public static function getPluginsInfo($filter = self::PLUGIN_INFO_ALL, $pathsFilter = false)
    {
        if (!defined('ABSPATH')) {
    ...
    public static function getWpPaths($pathKey = null, $homeAsAbs = false)
    {
        ...
        if (is_null($origPaths)) {
            ...
            if (is_multisite() && self::getMainSiteId() !== get_current_blog_id()) {
